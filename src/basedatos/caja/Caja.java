package basedatos.caja;

public class Caja {

    public String getId() {
        return id;
    }

    public String getUsuario() {
        return usuario;
    }

    public String getB200() {
        return b200;
    }

    public String getB100() {
        return b100;
    }

    public String getB50() {
        return b50;
    }

    public String getB20() {
        return b20;
    }

    public String getB10() {
        return b10;
    }

    public String getM5() {
        return m5;
    }

    public String getM2() {
        return m2;
    }

    public String getM1() {
        return m1;
    }

    public String getM050() {
        return m050;
    }

    public String getM020() {
        return m020;
    }

    public String getTotal_dinero() {
        return total_dinero;
    }

    public String getTotal_ventas() {
        return total_ventas;
    }

    public String getDescuadre() {
        return descuadre;
    }

    public String getFecha_fin_sesion() {
        return fecha_fin_sesion;
    }

    public String getHora_fin_sesion() {
        return hora_fin_sesion;
    }

    public void setId(String id) {
        this.id = id;
    }

    public void setUsuario(String usuario) {
        this.usuario = usuario;
    }

    public void setB200(String b200) {
        this.b200 = b200;
    }

    public void setB100(String b100) {
        this.b100 = b100;
    }

    public void setB50(String b50) {
        this.b50 = b50;
    }

    public void setB20(String b20) {
        this.b20 = b20;
    }

    public void setB10(String b10) {
        this.b10 = b10;
    }

    public void setM5(String m5) {
        this.m5 = m5;
    }

    public void setM2(String m2) {
        this.m2 = m2;
    }

    public void setM1(String m1) {
        this.m1 = m1;
    }

    public void setM050(String m050) {
        this.m050 = m050;
    }

    public void setM020(String m020) {
        this.m020 = m020;
    }

    public void setTotal_dinero(String total_dinero) {
        this.total_dinero = total_dinero;
    }

    public void setTotal_ventas(String total_ventas) {
        this.total_ventas = total_ventas;
    }

    public void setDescuadre(String descuadre) {
        this.descuadre = descuadre;
    }

    public void setFecha_fin_sesion(String fecha_fin_sesion) {
        this.fecha_fin_sesion = fecha_fin_sesion;
    }

    public void setHora_fin_sesion(String hora_fin_sesion) {
        this.hora_fin_sesion = hora_fin_sesion;
    }
    
    public Caja (String id) {
        this.id = id;
    }

    protected String id, usuario, b200, b100, b50, b20, b10, m5, m2, m1, m050, m020, total_dinero, total_ventas, descuadre, fecha_fin_sesion, hora_fin_sesion;

    
    
}
